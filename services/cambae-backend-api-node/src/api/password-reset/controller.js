...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Cambae Backend Api Node account.<br>
        ...
        &mdash; Cambae Backend Api Node Team
      `
      return sendMail({ toEmail: email, subject: 'Cambae Backend Api Node - Password Reset', content })
    })
